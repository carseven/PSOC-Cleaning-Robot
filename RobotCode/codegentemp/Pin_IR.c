...
#include "Pin_IR.h"
    ...
    #define Pin_IR_SetP4PinDriveMode(shift, mode)  \
    do { \
        Pin_IR_PC =   (Pin_IR_PC & \
                                (uint32)(~(uint32)(Pin_IR_DRIVE_MODE_IND_MASK << \
                                (Pin_IR_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (Pin_IR_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define Pin_IR_SetP4PinDriveMode(shift, mode)  \
        do { \
            Pin_IR_USBIO_CTRL_REG = (Pin_IR_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(Pin_IR_DRIVE_MODE_IND_MASK << \
                                    (Pin_IR_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (Pin_IR_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		Pin_IR_SetP4PinDriveMode(Pin_IR__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(Pin_IR_DR & (uint8)(~Pin_IR_MASK));
    drVal = (drVal | ((uint8)(value << Pin_IR_SHIFT) & Pin_IR_MASK));
    Pin_IR_DR = (uint32)drVal;
}
...
{
    return (uint8)((Pin_IR_PS & Pin_IR_MASK) >> Pin_IR_SHIFT);
}
...
{
    return (uint8)((Pin_IR_DR & Pin_IR_MASK) >> Pin_IR_SHIFT);
}
...
void Pin_IR_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  Pin_IR_INTCFG & (uint32)(~(uint32)position);
    Pin_IR_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(Pin_IR_INTSTAT & Pin_IR_MASK);
	Pin_IR_INTSTAT = maskedStatus;
    return maskedStatus >> Pin_IR_SHIFT;
}
